...
class Evolution(object):
	...
	def rand_normal(self, mean, stddev):
		if (not Evolution.n2_cached):
			...
			RAND_MAX = 2147483647
			x = 2.0 * np.random.randint(0, RAND_MAX)/RAND_MAX - 1
			y = 2.0 * np.random.randint(0, RAND_MAX)/RAND_MAX - 1
			r = x*x + y*y
			while (r == 0 or r > 1):
				RAND_MAX = 2147483647
				x = 2.0 * np.random.randint(0, RAND_MAX)/RAND_MAX - 1
	...
	def evolve(self, outfile   ):
		...
		while(self.num_evals < self.max_evals):
			...
			# print "Evals:", self.num_evals," Best fitness:", self.fitness[self.best_index], " Best Index:",self.best_index
			break
